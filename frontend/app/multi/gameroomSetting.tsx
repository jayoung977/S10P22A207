...
import { MultiRoomInfo, MultiRoom } from '@/public/src/stores/multi/MultiGameStore';
...
export default function GameRoomSetting () {
  const { data, isLoading , error } :UseQueryResult<MultiRoomInfo, Error> = useQuery('MultiRoomInfo',fetchMultiRoomInfo)
  ...
  const { result }: { result: MultiRoom[] | null} = data ? data: {result: null}
  console.log(result)
  return (
