...
export default function Stock({ id, index, data, isSelected, onClick }: any) {
  ...
  return (
    <div
      className={`hover:cursor-pointer grid grid-cols-8 items-center justify-center rounded-full bg-${bgColor} text-${textColor} hover:bg-small-1 hover:text-textColor-2 active:bg-small-11 active:text-textColor-2 my-5 mx-1 py-1`}
    ...
    >
      ...
      <div
        className={`${
          isHovered && "hidden"
        } col-span-4 grid grid-cols-4 items-center`}
      >
        <div className="col-span-2">{data.riseRate}%</div>
        <div className="col-span-2">{data.price}</div>
      </div>
