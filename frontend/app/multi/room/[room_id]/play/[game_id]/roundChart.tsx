...
function calculateEMA(data: any, period: number) {
  ...
  for (let i = period; i < data.length; i++) {
    let k = 2 / (period + 1);
    let ema: any =
      (data[i]?.endPrice - emaValues[i - 1][1]) * k + emaValues[i - 1][1];
    emaValues.push([data[i]?.date, parseFloat(ema)]);
  }
...
export default function RoundChart({ data }: any) {
  ...
  useEffect(() => {
    ...
    const handleShowAll = () => {
      chart.selectRange(
        purifiedData[0]?.date.split("T")[0],
        purifiedData[day + 299]?.date.split("T")[0]
      );
    };
    ...
    chart.selectRange(
      purifiedData[day + 249]?.date.split("T")[0],
      purifiedData[day + 299]?.date.split("T")[0]
    );
  ...
  return (
    <div className="row-span-9 grid grid-rows-12">
      ...
      <div
        ...
        className="row-span-11 flex items-center justify-center"
      ></div>
    </div>
  );
}
