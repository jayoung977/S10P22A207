...
import com.backend.api.global.jwt.dao.JwtRedisDao;
import com.backend.api.global.jwt.dto.TokenDto;
import com.backend.api.global.security.userdetails.CustomUserDetails;
import com.backend.api.global.security.userdetails.service.CustomUserDetailsService;
...
public class JwtService {
    ...
    public Authentication authenticateAccessToken(HttpServletRequest request) {
        ...
        log.info("토큰의 Claims에 저장된 권한:" + JwtClaimsParser.getPrivileges(claims));
        log.info("토큰의 Claims에 저장된 이메일:" + claims.getSubject());
        log.info("토큰의 Claims에 저장된 ID:" + claims.get("id"));
        Long id = Long.parseLong(claims.get("id").toString());
        UserDetails userDetails = CustomUserDetails.builder().email(claims.getSubject()).id(id)
    ...
    public String createAccessToken(CustomUserDetails userDetails) {
        ...
        return Jwts.builder().signWith(secretKey, Jwts.SIG.HS256).issuer(issuer).expiration(expirationDate)
                .subject(userDetails.getUsername()).claim("nickname", userDetails.getNickname()).claim("id", userDetails.getId())
    ...
    public String createRefreshToken(CustomUserDetails userDetails) {
        ...
        String refreshToken = Jwts.builder().signWith(secretKey, Jwts.SIG.HS256).issuer(issuer)
                .expiration(expirationDate).subject(userDetails.getUsername()).claim("id", userDetails.getId())
