...
public class MultiGameService {
	...
	public MultiGameStartResponseDto startMultiGame(Long memberId, MultiGameStartRequestDto dto) {
		...
		Long stockId = null;
		while (stockId == null) {
			List<String> stockIds = stockChartRepository.findDistinctStockCodeByDateBetween(randomDateTime,
				randomDateTime.plusDays(1));
			if (stockIds.isEmpty()) {
	...
	public MultiTradeResponseDto shortSelling(MultiTradeRequestDto dto, Long memberId) {
		...
		currentGame.updateShortAveragePrice(((dto.amount() * todayChart.getEndPrice()
			+ currentGame.getShortAveragePrice() * currentGame.getShortStockAmount()) / (dto.amount()
		...
		currentGame.addProfit(
			dto.amount() * (0.9975 * todayChart.getEndPrice() - currentGame.getAveragePrice())); // 수수료 고려
		currentGame.updateTotalAsset(totalAsset);
	...
	public MultiTradeResponseDto closeShortPosition(MultiTradeRequestDto dto, Long memberId) {
		...
		currentGame.updateCash(currentGame.getCash() + (long)(dto.amount() * todayChart.getEndPrice() * 0.9975));
		currentGame.addProfit(
			dto.amount() * (currentGame.getShortAveragePrice() - todayChart.getEndPrice() * 1.0025)); // 수수료 고려
		currentGame.updateTotalAsset(totalAsset);
		currentGame.addPurchaseAmount((long)dto.amount() * todayChart.getEndPrice());
		currentGame.updateAveragePrice(
			(dto.amount() * todayChart.getEndPrice() + currentGame.getAveragePrice() * currentGame.getStockAmount()) / (
				dto.amount() + currentGame.getStockAmount()));
	...
	public MultiNextDayResponseDto getTomorrow(MultiNextDayRequestDto dto, Long memberId) {
		...
		StockChart todayChart = stockChartRepository.findById(currentGame.getFirstDayStockChartId() + 299 + dto.day())
			.orElseThrow();
		StockChart yesterdayChart = stockChartRepository.findById(
			currentGame.getFirstDayStockChartId() + 299 + dto.day() - 1).orElseThrow();

		// 어제에 비해서 얼마나 바뀌었는지. 매수 수량은 더해주고
		// 공매도는 반대.
		currentGame.addProfit(
			(currentGame.getStockAmount() - currentGame.getShortStockAmount()) * (todayChart.getEndPrice()
				- yesterdayChart.getEndPrice()));
	...
	public MultiLogResponseDto getMultiGameLog(Long multiGameLogId, Long memberId) {
		...
		List<MultiLogMemberDto> multiLogMemberDtoList = multiGamePlayerRepository.findAllByMultiGameLog_Id(
			multiGameLogId).stream().map(
	...
	private List<MultiLogTradeDto> getMultiLogTradeList(Long multiGameLogId, Long memberId) {
		List<MultiTrade> multiTradeList = multiTradeRepository.findAllByMultiGameLog_IdAndMemberId(multiGameLogId,
			memberId);
