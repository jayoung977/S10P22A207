...
export default function AssetHeld({ data }: any) {
  ...
  const idx: number = stockListData.findIndex(
    (x: any) => x?.stockId == data?.stockId
  );
  return (
    <div
      onClick={() => {
        ...
        <div className="row-span-1 grid grid-cols-4 justify-center">
          ...
          <div
            className={`col-span-1 m-auto ${
              data?.unrealizedProfit > 0 ? "text-red-500" : "text-blue-500"
            }`}
          >
            {data?.unrealizedProfit.toLocaleString()}원
          </div>
